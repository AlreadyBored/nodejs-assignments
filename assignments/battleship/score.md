...
- **-95% of total task score** any external tools except `ws`, `cross-env`, `dotenv`, `tsx`, `typescript`, `ts-node`, `ts-node-dev`, `nodemon`, `linter and formatter` and its plugins, `bundler` and its plugins, `@types/*` and testing tools (for example, Vitest, Jest, Mocha, AVA, Jasmine, Cypress, Storybook, Puppeteer)
