...
- **-95% of total task score** any external tools except `ws`, `nutjs.dev`, `jimp`, `cross-env`, `dotenv`, `typescript`, `ts-node`, `ts-node-dev`, `nodemon`, `eslint` and its plugins, `bundler` and its plugins, `prettier`, `@types/*` and testing tools (for example, Vitest, Jest, Mocha, AVA, Jasmine, Cypress, Storybook, Puppeteer)
